            ...
            foreach (Node nProperty in nProperties.subnodes)
            {
                switch (nProperty.propertyValues[0])
                {
                    ...
                    case "WhammyBarDestinationOffset":
                        ...
                        break;
                    default:
                        throw new InvalidOperationException($"{nameof(transferBeat)} Unrecognized node property: {nProperty.propertyValues[0]}");
                        break;
                }
            }
            ...
            foreach (Node nProperty in nProperties.subnodes)
            {
                switch (nProperty.propertyValues[0])
                {
                    ...
                    case "Tapped":
                        ...
                        break;
                    default:
                        throw new InvalidOperationException($"{nameof(transferNote)} Unrecognized node property: {nProperty.propertyValues[0]}");
                        break;
                }
