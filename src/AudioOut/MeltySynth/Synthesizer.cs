...
namespace MeltySynth
{
    ...
    public sealed class Synthesizer : IAudioRenderer
    {
        ...
        /// Stops a note.
        /// </summary>
        /// <param name="voice">The note voice.</param>
        public void NoteOff(Voice note)
        {
            note.End();
        }
        ...
        public List<Voice> NoteOn(int channel, int key, int velocity)
        {
            if (velocity == 0)
            {
                NoteOff(channel, key);
                return new List<Voice>(0);
            }
            ...
            // Typically L + R
            List<Voice> addedVoices = new List<Voice>(2);
